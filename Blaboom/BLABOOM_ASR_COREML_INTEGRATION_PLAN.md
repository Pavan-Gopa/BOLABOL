# Blaboom — план интеграции ASR (Core ML): GigaAM + Canary 1B + Canary Flash
...
Встроить в Blaboom **три локальных ASR-модели на Core ML** (ANE), с полным UX: Local Models (каталог, download, presence), выбор engine, диктовка, Help/Settings/i18n, плюс **пересборка онбординга** (UI language / primary / additional без путаницы).
...
```swift
// NativeBlaboomCore — pure function, unit-tested
...
Имена для `STATE.yaml` / tags: `asr/pre-S0` … или `blaboom/pre-S0`.
...
- Body: Choose the language for Blaboom’s menus, settings, and help.  
...
5. Tags: `blaboom/pre-S*`, `blaboom/S*-done`.
