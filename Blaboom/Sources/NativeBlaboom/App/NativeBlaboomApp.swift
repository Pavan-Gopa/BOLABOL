    ...
    var body: some Scene {
        Window("BLABOOM!", id: "main") {
            ContentView()
                ...
                .environment(\.nativeBlaboomUIScale, generalSettingsStore.settings.uiScale)
...
extension AppDelegate {
    func configureApplicationIcon() {
        if let logo = bundledImage(named: "BLABOOM_LOGO", extension: "svg", subdirectory: "Logos") {
    ...
    func observeWindowLifecycle() {
        ...
        NotificationCenter.default.addObserver(
            ...
            name: .nativeBlaboomSettingsHotkeyTriggered,
            object: nil
        )
    }
    ...
    func captureMainWindowIfNeeded() {
        ...
        guard let window = NSApp.windows.first(where: isPrimaryBlaboomWindow(_:)) else { return }
        registerMainWindowIfNeeded(window)
    }

    func registerMainWindowIfNeeded(_ window: NSWindow) {
        guard isPrimaryBlaboomWindow(window) else { return }
    ...
    func isPrimaryBlaboomWindow(_ window: NSWindow) -> Bool {
        !window.isKind(of: NSPanel.self) && window.title == "BLABOOM!"
    }

    func configureMainWindowTitle(_ window: NSWindow) {
        window.title = "BLABOOM!"
        window.titleVisibility = .visible
    ...
    func statusItemImage() -> NSImage? {
        ...
        let fallback = NSImage(systemSymbolName: "waveform", accessibilityDescription: "Blaboom")
    ...
    @objc func closeAllWindowsToTray() {
        NotificationCenter.default.post(name: .nativeBlaboomDismissSheets, object: nil)
