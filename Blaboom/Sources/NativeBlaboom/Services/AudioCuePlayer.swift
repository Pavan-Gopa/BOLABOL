...
final class AudioCuePlayer {
    ...
    func play(_ cue: Cue, settings: OverlayHUDSettings) {
        ...
        do {
            ...
            NativeBlaboomLog.hotkey.warning("AVAudioPlayer refused cue=\(cue.soundName, privacy: .public)")
            Self.appendDebugLog("AVAudioPlayer refused cue=\(cue.soundName)")
        } catch {
            NativeBlaboomLog.hotkey.error("Audio cue failed cue=\(cue.soundName, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
        ...
        if let sound = nsSound(for: cue) {
            ...
            NativeBlaboomLog.hotkey.warning("NSSound refused cue=\(cue.soundName, privacy: .public)")
    ...
    private func playWithAFPlay(_ cue: Cue, volume: Double) -> Bool {
        do {
            ...
            NativeBlaboomLog.hotkey.info("Started afplay cue=\(cue.soundName, privacy: .public) volume=\(volume, privacy: .public)")
            Self.appendDebugLog("afplay started cue=\(cue.soundName) volume=\(volume) pid=\(process.processIdentifier)")
            return true
        } catch {
            NativeBlaboomLog.hotkey.error("afplay cue failed cue=\(cue.soundName, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
    ...
    private func cueFileURL(for cue: Cue) throws -> URL {
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("NativeBlaboomAudioCues", isDirectory: true)
    ...
    private static func appendDebugLog(_ line: String) {
        ...
        do {
            ...
            NativeBlaboomLog.hotkey.error("HUD audio debug log write failed error=\(error.localizedDescription, privacy: .public)")
        }
    }
