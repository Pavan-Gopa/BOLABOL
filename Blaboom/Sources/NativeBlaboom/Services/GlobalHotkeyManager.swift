...
extension Notification.Name {
    static let nativeBlaboomHotkeyTriggered = Notification.Name("nativeBlaboomHotkeyTriggered")
    static let nativeBlaboomHotkeyKeyDown = Notification.Name("nativeBlaboomHotkeyKeyDown")
    static let nativeBlaboomHotkeyKeyUp = Notification.Name("nativeBlaboomHotkeyKeyUp")
    static let nativeBlaboomTargetHotkeyTriggered = Notification.Name("nativeBlaboomTargetHotkeyTriggered")
    static let nativeBlaboomQuickTranslationHotkeyTriggered = Notification.Name("nativeBlaboomQuickTranslationHotkeyTriggered")
    static let nativeBlaboomSettingsHotkeyTriggered = Notification.Name("nativeBlaboomSettingsHotkeyTriggered")
    static let nativeBlaboomDismissSheets = Notification.Name("nativeBlaboomDismissSheets")
}
...
final class GlobalHotkeyManager {
    ...
    private func register(primary: String, secondary: String, tertiary: String, settings: String) throws {
        ...
        if secondaryStatus == noErr, let secondaryRef {
            ...
            NativeBlaboomLog.models.warning(
        ...
        if let settingsCombo = try? HotkeyCombination(settings) {
            ...
            if registeredCombos.contains(settingsKey) {
                ...
                if settingsStatus == noErr, let settingsRef {
                    ...
                    NativeBlaboomLog.models.info("Settings hotkey registered successfully")
                } else {
                    NativeBlaboomLog.models.warning(
    ...
    private func installLocalMonitor() {
        localEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if self.isSettingsHotkeyEvent(event) {
                Task { @MainActor in
                    NotificationCenter.default.post(name: .nativeBlaboomSettingsHotkeyTriggered, object: nil)
                }
                return nil
            }
            return event
    ...
    private func installEventHandler() {
                    ...
                    if hotKeyID.id == 1 {
                        Task { @MainActor in
                            if eventKind == UInt32(kEventHotKeyPressed) {
                                NotificationCenter.default.post(name: .nativeBlaboomHotkeyKeyDown, object: nil)
                                NotificationCenter.default.post(name: .nativeBlaboomHotkeyTriggered, object: nil)
                            } else if eventKind == UInt32(kEventHotKeyReleased) {
                                NotificationCenter.default.post(name: .nativeBlaboomHotkeyKeyUp, object: nil)
                            }
                        }
                    } else if hotKeyID.id == 2 && eventKind == UInt32(kEventHotKeyPressed) {
                        Task { @MainActor in
                            NotificationCenter.default.post(name: .nativeBlaboomTargetHotkeyTriggered, object: nil)
                        }
                    } else if hotKeyID.id == 3 && eventKind == UInt32(kEventHotKeyPressed) {
                        Task { @MainActor in
                            NotificationCenter.default.post(name: .nativeBlaboomQuickTranslationHotkeyTriggered, object: nil)
                        }
                    } else if hotKeyID.id == 4 && eventKind == UInt32(kEventHotKeyPressed) {
                        NativeBlaboomLog.models.info("Settings hotkey triggered (ID 4)")
                        Task { @MainActor in
                            NotificationCenter.default.post(name: .nativeBlaboomSettingsHotkeyTriggered, object: nil)
                        }
                    }
        ...
        if status != noErr {
            NativeBlaboomLog.models.error("Failed to install global hotkey event handler: \(status)")
        }
    }
}
