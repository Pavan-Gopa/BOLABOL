...
struct ContentView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomHotkeyKeyDown)) { _ in
                handleHotkeyKeyDown()
            }
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomHotkeyKeyUp)) { _ in
                handleHotkeyKeyUp()
            }
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomTargetHotkeyTriggered)) { _ in
                openFloatingTranslationWindow()
            }
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomQuickTranslationHotkeyTriggered)) { _ in
                openQuickTranslationWindow()
            }
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomSettingsHotkeyTriggered)) { _ in
                AppDelegate.shared?.cycleSettingsAndMainWindow()
            }
            .onReceive(NotificationCenter.default.publisher(for: .nativeBlaboomDismissSheets)) { _ in
    ...
    private func updateRawText(noteID: BlaboomNote.ID, text: String) {
    ...
    private func importAudioFile(_ url: URL) {
        Task { @MainActor in
            do {
                ...
                NativeBlaboomLog.transcription.error("Audio file import failed: \(error.localizedDescription)")
            }
        }
    }
            ...
            if let supportedType {
                provider.loadFileRepresentation(forTypeIdentifier: supportedType.identifier) { url, _ in
                    ...
                    do {
                        ...
                        NativeBlaboomLog.transcription.error("Audio drop copy failed: \(error.localizedDescription)")
                    }
                }
                return true
            }
    ...
    private func translateText(
        ...
        let startedAt = Date()
        NativeBlaboomLog.polishing.info(
    ...
    private func transcribeRecording(
        ...
        Task { @MainActor in
            ...
            guard polishingEngineStore.canAutoPolishAfterTranscription else {
                NativeBlaboomLog.polishing.info("Skipped automatic polishing because no local polishing model is ready.")
    ...
    private func polishNote(_ noteID: BlaboomNote.ID) {
    ...
    private func autoTranslateRawText(
        ...
        do {
            ...
            NativeBlaboomLog.polishing.error(
    ...
    private func transcribeRecordingViaGeminiCloud(
        ...
        do {
            ...
            NativeBlaboomLog.transcription.error(
    ...
    private func markPolishingUnavailable(for noteID: BlaboomNote.ID) {
