...
struct NoteDetailView: View {
    ...
    let onPolishRequested: (BlaboomNote.ID) -> Void
    let onMarkdownRequested: (BlaboomNote.ID, ProcessingVariant) -> Void
    let onTextChanged: (BlaboomNote.ID, ProcessingVariant, String) -> Void
    ...
    private func header(for note: BlaboomNote) -> some View {
        HStack(spacing: 8) {
    ...
    private func topConfigBar(for note: BlaboomNote) -> some View {
    ...
    private func bottomBar(for note: BlaboomNote) -> some View {
    ...
    private func bottomActionIcons(for note: BlaboomNote) -> some View {
        HStack(spacing: 4) {
            toolbarIconButton(
            ...
            .buttonStyle(BlaboomToolbarButtonStyle(size: 34))
            .labelStyle(.iconOnly)
    ...
    private func emptyStateActionButton(
        ...
        Button(action: action) {
            Label(title, systemImage: systemImage)
                .blaboomFont(.caption, weight: .medium)
    ...
    private func polishingOverview(for note: BlaboomNote) -> some View {
        HStack(spacing: 8) {
    ...
    private func isPolishing(_ note: BlaboomNote) -> Bool {
        [ProcessingVariant.variantOne, .variantTwo].contains { variant in
            note.polishingStatus(for: variant).phase == .polishing
        }
    }

    private func isImportedAudioTranscribing(_ note: BlaboomNote) -> Bool {
        note.transcriptionStatus.phase == .transcribing
            && note.audioRecording?.source == .importedFile
    }

    @ViewBuilder
    private func polishingStatus(for note: BlaboomNote, variant: ProcessingVariant) -> some View {
        if variant != .raw {
            let status = note.polishingStatus(for: variant)
    ...
    private func markdownButtonDisabled(for note: BlaboomNote, variant: ProcessingVariant) -> Bool {
    ...
    private func placeholderText(for note: BlaboomNote, variant: ProcessingVariant) -> String {
        switch variant {
        case .raw:
    ...
    private func text(for note: BlaboomNote, variant: ProcessingVariant) -> String {
        switch variant {
        case .raw:
            note.rawText
        case .variantOne:
    ...
    private func bindingText(for note: BlaboomNote, variant: ProcessingVariant) -> Binding<String> {
        Binding(
    ...
    private func toolbarIconButton(
        ...
        .buttonStyle(BlaboomToolbarButtonStyle())
        .labelStyle(.iconOnly)
        .disabled(isDisabled)
        .help(helpText)
    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                ...
                if showsMarkdownAction {
                    ...
                    .buttonStyle(BlaboomToolbarButtonStyle(size: 28))
