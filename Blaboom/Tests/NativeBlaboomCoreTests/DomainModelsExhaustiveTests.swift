...
@Test
func blaboomNotePolishingStatusDefaultsToIdle() {
  let note = BlaboomNote(title: "T", rawText: "r")
  #expect(note.polishingStatus(for: .variantOne) == .idle)
  #expect(note.polishingStatus(for: .variantTwo).phase == .idle)
}

@Test
func blaboomNoteBestDisplayTextPrefersRawThenVariants() {
  #expect(BlaboomNote(title: "t", rawText: " raw ").bestDisplayText() == "raw")
  #expect(
    BlaboomNote(title: "t", rawText: "", polishedVariantOne: " v1 ").bestDisplayText() == "v1"
  )
  #expect(
    BlaboomNote(title: "t", rawText: "  ", polishedVariantOne: "", polishedVariantTwo: "v2")
      .bestDisplayText() == "v2"
  )
  #expect(BlaboomNote(title: "t", rawText: "").bestDisplayText().isEmpty)
}

@Test
func blaboomNotePreviewTextTruncatesAndFallsBackToTitle() {
  let long = String(repeating: "word ", count: 40)
  let note = BlaboomNote(title: "Title", rawText: long)
  let preview = note.previewText(maxLength: 20)
  #expect(preview.hasSuffix("..."))
  #expect(preview.count <= 20)

  let empty = BlaboomNote(title: "  My Title  ", rawText: "")
  // previewText returns the original title when body is empty (trim only for emptiness check).
  #expect(empty.previewText() == "  My Title  ")

  let blank = BlaboomNote(title: "   ", rawText: "")
  #expect(blank.previewText() == AppText.localized(.blankNoteFallback, language: .english))
}

@Test
func blaboomNoteCopyAllTextJoinsChronologicallyAndSkipsEmpty() {
  let older = BlaboomNote(
    title: "a",
    createdAt: Date(timeIntervalSince1970: 100),
    rawText: "first"
  )
  let newer = BlaboomNote(
    title: "b",
    createdAt: Date(timeIntervalSince1970: 200),
    rawText: "",
    polishedVariantOne: "second"
  )
  let empty = BlaboomNote(
    title: "c",
...
@Test
func blaboomNoteCodableRoundTripPreservesStatuses() throws {
  var note = BlaboomNote(title: "n", rawText: "raw")
  note.transcriptionStatus = .completed(backendName: "Whisper")
  note.polishingStatuses[.variantOne] = .failed(message: "err")
  let data = try JSONEncoder().encode(note)
  let decoded = try JSONDecoder().decode(BlaboomNote.self, from: data)
...
@Test
func blaboomNotePreviewArrayIsNonEmpty() {
  #expect(!Array<BlaboomNote>.preview.isEmpty)
  #expect(Array<BlaboomNote>.preview.allSatisfy { !$0.rawText.isEmpty })
}
...
func audioRecordingDefaultsAndLegacyDecode() throws {
  let url = URL(fileURLWithPath: "/tmp/blaboom-test.wav")
...
@Test
func nativeBlaboomLogSubsystemIsStable() {
  #expect(NativeBlaboomLog.subsystem == "com.pavan.NativeBlaboom")
}
