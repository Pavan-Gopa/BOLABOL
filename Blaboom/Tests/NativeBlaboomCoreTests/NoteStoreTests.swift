...
func noteStoreSelectsFirstNoteOnInitialization() {
    let first = BlaboomNote(
        id: UUID(uuidString: "00000000-0000-0000-0000-000000000001")!,
        title: "First",
        rawText: "One"
    )
    let second = BlaboomNote(
...
func noteStoreAddsRecordedNoteAtTopAndSelectsIt() {
    ...
    let recording = AudioRecording(
        ...
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-test.wav"),
        createdAt: now,
        duration: 12.5,
        sampleRate: 48_000,
        channelCount: 1,
...
func noteStoreClearsAllNotesAndSelection() {
    let store = NoteStore(notes: [
        BlaboomNote(title: "One", rawText: "One"),
        BlaboomNote(title: "Two", rawText: "Two")
    ])
...
func noteStoreClearAllAndPurgeRemoveMicrophoneRecordingsAndOrphans() throws {
    ...
    let root = fileManager.temporaryDirectory
        .appendingPathComponent("NativeBlaboom-note-store-audio-\(UUID().uuidString)", isDirectory: true)
    ...
    let store = NoteStore(
        notes: [
            BlaboomNote(title: "Voice", rawText: "hi", audioRecording: recording)
        ],
...
@Test
func blaboomNotePreviewTextPrefersRawTranscript() {
...
@Test
func blaboomNotePreviewTextUsesLocalizedBlankFallbackWhenNoteHasNoTitleOrText() {
    let note = BlaboomNote(title: "   ", rawText: "")

    #expect(note.previewText() == AppText.localized(.blankNoteFallback, language: .english))
}

@Test
func blaboomNotesCopyAllTextJoinsNonEmptyOutputs() {
    let notes = [
        BlaboomNote(
            title: "Newest",
