...
func recordingTranscriptionWorkflowCreatesNoteAndStoresTranscript() async {
    ...
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-workflow.wav"),
    ...
    let engine = SuccessfulTranscriptionEngine(
        ...
        expectedLastPathComponent: "native-blaboom-workflow.wav"
    )
...
func recordingTranscriptionWorkflowStoresFailureStatus() async {
    let store = NoteStore()
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-failure.wav"),
...
func recordingTranscriptionWorkflowPassesForcedLanguageCodeToEngine() async {
    let store = NoteStore()
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-language.wav"),
...
func recordingTranscriptionWorkflowPassesTranslateToEnglishToEngine() async {
    let store = NoteStore()
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-translate.wav"),
...
func recordingTranscriptionWorkflowAppliesEnabledGlossaryBeforeSavingTranscript() async {
    let store = NoteStore()
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-glossary.wav"),
...
func recordingTranscriptionWorkflowUsesAuthorTranscriptionLanguageGlossaryTarget() async {
    let store = NoteStore()
    let recording = AudioRecording(
        fileURL: URL(fileURLWithPath: "/tmp/native-blaboom-author-language-glossary.wav"),
