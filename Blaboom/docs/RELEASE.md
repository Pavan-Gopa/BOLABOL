...
Repository: `https://github.com/Pavan-Gopa/Blaboom`.
...
- `dist/Blaboom.dmg` — signed (and optionally notarized) disk image  
- `dist/release/Blaboom.app` — app bundle  
...
```bash
xcrun notarytool store-credentials "Blaboom-Notary" \
...
```bash
./script/notarize_dmg.sh dist/Blaboom.dmg
...
```bash
spctl -a -vv -t install dist/Blaboom.dmg
xcrun stapler validate dist/Blaboom.dmg
```

## CLI install for recipients

```bash
./script/install.sh /path/to/Blaboom.dmg
...
gh repo view Pavan-Gopa/Blaboom --json isPrivate
...
1. Launch `/Applications/Blaboom.app`  
