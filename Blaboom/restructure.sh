#!/bin/bash
# ============================================================
# BLABOOM — Restructure: Remove Electron, Flatten Directory
# ============================================================
# This script:
# 1. Removes Electron version (if exists)
# 2. Moves all content from NativeAppleSilicon/ to Blaboom/ root
...
PROJECT_ROOT="/Users/pavan/Documents/AI Projects/Blaboom"
NATIVE_DIR="$PROJECT_ROOT/NativeAppleSilicon"

echo "🚀 BLABOOM Restructure Script"
...
for skill_file in "${SKILL_FILES[@]}"; do
    if [ -f "$skill_file" ]; then
        sed -i '' 's|/Users/pavan/Documents/AI Projects/Blaboom/NativeAppleSilicon|/Users/pavan/Documents/AI Projects/Blaboom|g' "$skill_file"
...
echo "🔨 Testing build..."
cd "$PROJECT_ROOT"
if swift build > /tmp/blaboom_build.log 2>&1; then
    echo "✅ Build successful"
else
    echo "⚠️  Build failed (see /tmp/blaboom_build.log)"
    tail -10 /tmp/blaboom_build.log
fi
echo ""

# ---- Step 7: Run tests ----
echo "🧪 Running tests..."
if swift test > /tmp/blaboom_test.log 2>&1; then
    echo "✅ Tests passed"
    grep "Test run with" /tmp/blaboom_test.log | tail -1
else
    echo "⚠️  Tests failed (see /tmp/blaboom_test.log)"
    tail -10 /tmp/blaboom_test.log
fi
echo ""
