...
SWIFT_PRODUCT_NAME="NativeBlaboom"
APP_NAME="Blaboom"
...
WORKER_NAME="NativeBlaboomPolishWorker"
...
DEFAULT_CODESIGN_IDENTITY="NativeBlaboom Local Development"

cd "$ROOT_DIR"

echo "=== Cleaning up existing processes ==="
pkill -f "NativeBlaboom/Models/Polishing/HuggingFace/Direct" >/dev/null 2>&1 || true
...
ICON_SRC="$ROOT_DIR/Sources/NativeBlaboom/Resources/AppIcon.icns"
if [[ ! -f "$ICON_SRC" ]]; then
...
mkdir -p "$APP_LOGOS"
for svg_name in BLABOOM_LOGO.svg BLABOOM_LOGO_Full.svg BLABOOM_Wordmark.svg BLABOOM_status_bar_icon.svg; do
  svg_src="$ROOT_DIR/Sources/NativeBlaboom/Resources/Logos/$svg_name"
  if [[ -f "$svg_src" ]]; then
...
<dict>
  ...
  <string>Blaboom needs microphone access to record and transcribe speech.</string>
  <key>NSSpeechRecognitionUsageDescription</key>
  <string>Blaboom uses on-device speech recognition to transcribe recorded audio.</string>
  <key>NSAppleEventsUsageDescription</key>
  <string>Blaboom needs permission to paste transcribed text into the active app.</string>
</dict>
</plist>
PLIST
...
# Output DMG path
OUTPUT_DMG="$DIST_DIR/Blaboom.dmg"
rm -f "$OUTPUT_DMG"
...
mkdir -p "$HANDOFF_DIR"
cp "$OUTPUT_DMG" "$HANDOFF_DIR/Blaboom.dmg"
cp "$ROOT_DIR/script/install.sh" "$HANDOFF_DIR/install.sh"
chmod +x "$HANDOFF_DIR/install.sh"
(
  cd "$HANDOFF_DIR"
  shasum -a 256 Blaboom.dmg install.sh > SHA256SUMS.txt
)
...
ls -lh "$OUTPUT_DMG" "$HANDOFF_DIR/Blaboom.dmg"
