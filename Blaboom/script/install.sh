#!/usr/bin/env bash
# Install Blaboom.app into /Applications from a DMG (local path or latest private GitHub release).
set -euo pipefail

APP_NAME="Blaboom"
VOLUME_NAME="Blaboom"
INSTALL_DIR="${INSTALL_DIR:-/Applications}"
REPO="${BLABOOM_REPO:-Pavan-Gopa/Blaboom}"
DMG_PATTERN="${BLABOOM_DMG_PATTERN:-Blaboom*.dmg}"
OPEN_AFTER=0
FROM_GITHUB=0
DMG_PATH=""
MOUNT_POINT=""
...
usage() {
  cat <<'USAGE'
Usage:
  ./script/install.sh /path/to/Blaboom.dmg [--open]
  ./script/install.sh --from-github [--open]

Options:
  --from-github   Download the latest Blaboom*.dmg from the private GitHub release
...
Environment:
  ...
  BLABOOM_REPO         GitHub repo (default: Pavan-Gopa/Blaboom)
  BLABOOM_DMG_PATTERN  Release asset glob (default: Blaboom*.dmg)
USAGE
}
...
if [[ "$FROM_GITHUB" -eq 1 ]]; then
  ...
  TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/blaboom-install.XXXXXX")"
...
# Free a previous Blaboom volume if a prior install left it mounted.
...
if [[ -z "$MOUNT_POINT" || ! -d "$MOUNT_POINT/$APP_NAME.app" ]]; then
  for candidate in "/Volumes/$VOLUME_NAME" /Volumes/Blaboom*; do
...
if [[ ! -x "$DEST/Contents/MacOS/Blaboom" ]]; then
  echo "warning: could not confirm executable Contents/MacOS/Blaboom" >&2
fi
