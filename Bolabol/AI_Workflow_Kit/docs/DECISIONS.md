...
## ADR-023 - Non-destructive release code hygiene boundary

**Status:** Proposed - 2026-08-09

### Context

Bolabol 1.0.4 needs release hygiene, but the repository also contains persisted
legacy names, historical rename/spike evidence, public Core APIs, ASR/HUD timing
seams, and large orchestration files. Treating every old-looking name or large
file as removable would risk data loss or behavior changes.

### Decision

- A hygiene deletion requires both repository reference proof and Graphify
  evidence showing no external runtime caller. Public APIs, persisted/Codable
  values, migration/decode paths, and user/model data are never dead solely
  because the current app has no caller.
- The campaign may remove proven unreachable private/internal code, correct
  current comments/copy/release metadata, remove compiler-neutral imports or
  redundant modifiers, and move one already-isolated internal module without
  changing declarations, lifecycle, or visibility.
- Active 1.0.4 docs may be corrected. Historical ADRs, plans, spike reports,
  rename evidence, FEEDBACK, UserData, and Graphify memories remain historical
  evidence and are not globally rewritten.
- Persistence keys, Application Support paths, Keychain services, bundle
  identity, Codable values, public API, dependencies, routing, ASR decode,
  Core ML/chunking/model math, HUD geometry/hit testing, and concurrency/timing
  changes require a separate accepted campaign.
- Coder may implement only Human/Orchestrator-accepted HYG IDs from
  `AI_Workflow_Kit/docs/AI/ARCHITECT_HANDOFF.md`. Coder does not commit, tag,
  push, publish, or delete UserData/model assets.

### Acceptance

`swift test`, `./script/qa/run_all.sh`, and a clean Release verify must pass.
The Coder handoff must enumerate applied/skipped HYG IDs and show that the built
bundle has Bolabol 1.0.4 identity with no stale release artifact.
