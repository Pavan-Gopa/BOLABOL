#!/usr/bin/env bash
# graphify_rebuild.sh — rebuild Bolabol knowledge graph (agent token savings)
#
...
OUT_DIR="$BOLABOL_ROOT/graphify-out"
...
echo "Rebuilding Bolabol knowledge graph"
echo "  product root: $BOLABOL_ROOT"
echo "  output:       $GRAPH_JSON"
mkdir -p "$OUT_DIR"

# Update code graph under Bolabol only (not whole monorepo).
graphify update "$BOLABOL_ROOT" "${EXTRA_ARGS[@]}"
