...
final class HotkeyOutputDispatcher {
    ...
    func dispatch(
        ...
        guard !trimmed.isEmpty else {
            NativeBolabolLog.hotkey.warning("Skipped hotkey output because resolved text is empty.")
            return
        }
    ...
    private func pasteIntoActiveApp(
        ...
        if !hasAccessibilityPermission {
            NativeBolabolLog.hotkey.warning("Accessibility permission is required to paste hotkey output into the active app.")
        ...
        if let targetApplication {
            ...
            NativeBolabolLog.hotkey.warning("No captured target app for hotkey paste; using current frontmost app.")
        }
        ...
        Task { @MainActor in
            ...
            guard hasAccessibilityPermission else {
                NativeBolabolLog.hotkey.warning("Cannot send synthetic Cmd-V because Accessibility is not trusted.")
                return
            }
    ...
    private func attemptAXInsertion(
        ...
        NativeBolabolLog.hotkey.info("AX insertion did not modify the field; will fall back to clipboard paste.")
        return false
    }
    ...
    private func resolveInsertionElement(
        ...
        NativeBolabolLog.hotkey.warning("Could not resolve any AX element for text insertion.")
        return nil
    }
    ...
    private func insert(_ text: String, into element: AXUIElement, source: String) -> Bool {
        ...
        NativeBolabolLog.hotkey.info("Inserted hotkey output through AX value source=\(source, privacy: .public).")
        return true
    }
