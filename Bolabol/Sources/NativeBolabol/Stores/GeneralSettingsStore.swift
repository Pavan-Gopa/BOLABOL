...
final class GeneralSettingsStore: ObservableObject {
    ...
    func testOverlayHUDSounds() {
        ...
        logExportMessage = "HUD audio debug log: ~/Library/Application Support/NativeBolabol/Logs/hud-audio.log"
    }
...
private enum SystemLogExporter {
    static func exportRecentLogs() throws -> URL {
        ...
        let outputURL = logsDirectory.appendingPathComponent("NativeBolabol-system.log")
        let predicate = "subsystem == \"\(NativeBolabolLog.subsystem)\""
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "NativeBolabolLogs", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: message])
        }

        let header = """
        NativeBolabol system logs
