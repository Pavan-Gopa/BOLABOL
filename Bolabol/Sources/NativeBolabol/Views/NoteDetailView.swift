...
struct NoteDetailView: View {
    ...
    let onPolishRequested: (BolabolNote.ID) -> Void
    let onMarkdownRequested: (BolabolNote.ID, ProcessingVariant) -> Void
    let onTextChanged: (BolabolNote.ID, ProcessingVariant, String) -> Void
    ...
    private func header(for note: BolabolNote) -> some View {
        HStack(spacing: 8) {
    ...
    private func topConfigBar(for note: BolabolNote) -> some View {
    ...
    private func bottomBar(for note: BolabolNote) -> some View {
        ControlSurface(cornerRadius: 12) {
            HStack(spacing: 8) {
                ...
                .frame(minWidth: 40, idealWidth: 160, maxWidth: 240)
                .frame(height: 42)
                .layoutPriority(0)

                Spacer(minLength: 2)

                bottomActionIcons(for: note)
            }
        }
    }

    private func bottomActionIcons(for note: BolabolNote) -> some View {
        HStack(spacing: 4) {
            toolbarIconButton(
            ...
            .buttonStyle(BolabolToolbarButtonStyle(size: 34))
            .labelStyle(.iconOnly)
    ...
    private func emptyStateActionButton(
        ...
        Button(action: action) {
            Label(title, systemImage: systemImage)
                .bolabolFont(.caption, weight: .medium)
    ...
    private func polishingOverview(for note: BolabolNote) -> some View {
        HStack(spacing: 8) {
    ...
    private func isPolishing(_ note: BolabolNote) -> Bool {
        [ProcessingVariant.variantOne, .variantTwo].contains { variant in
            note.polishingStatus(for: variant).phase == .polishing
        }
    }

    private func isImportedAudioTranscribing(_ note: BolabolNote) -> Bool {
        note.transcriptionStatus.phase == .transcribing
            && note.audioRecording?.source == .importedFile
    }

    @ViewBuilder
    private func polishingStatus(for note: BolabolNote, variant: ProcessingVariant) -> some View {
        if variant != .raw {
            let status = note.polishingStatus(for: variant)
    ...
    private func markdownButtonDisabled(for note: BolabolNote, variant: ProcessingVariant) -> Bool {
    ...
    private func placeholderText(for note: BolabolNote, variant: ProcessingVariant) -> String {
        switch variant {
        case .raw:
    ...
    private func text(for note: BolabolNote, variant: ProcessingVariant) -> String {
        switch variant {
        case .raw:
            note.rawText
        case .variantOne:
    ...
    private func bindingText(for note: BolabolNote, variant: ProcessingVariant) -> Binding<String> {
        Binding(
    ...
    private func toolbarIconButton(
        ...
        .buttonStyle(BolabolToolbarButtonStyle())
        .labelStyle(.iconOnly)
        .disabled(isDisabled)
        .help(helpText)
    }
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                Label(title, systemImage: variant.systemImage)
                    .bolabolFont(.headline, weight: .semibold)
                ...
                if showsMarkdownAction {
                    ...
                    .buttonStyle(BolabolToolbarButtonStyle(size: 28))
