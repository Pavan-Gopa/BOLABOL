#!/usr/bin/env bash
# Install Bolabol.app into /Applications from a DMG (local path or latest private GitHub release).
set -euo pipefail

APP_NAME="Bolabol"
VOLUME_NAME="Bolabol"
INSTALL_DIR="${INSTALL_DIR:-/Applications}"
REPO="${BOLABOL_REPO:-Pavan-Gopa/Bolabol}"
DMG_PATTERN="${BOLABOL_DMG_PATTERN:-Bolabol*.dmg}"
OPEN_AFTER=0
FROM_GITHUB=0
DMG_PATH=""
MOUNT_POINT=""
...
usage() {
  cat <<'USAGE'
Usage:
  ./script/install.sh /path/to/Bolabol.dmg [--open]
  ./script/install.sh --from-github [--open]

Options:
  --from-github   Download the latest Bolabol*.dmg from the private GitHub release
...
Environment:
  ...
  BOLABOL_REPO         GitHub repo (default: Pavan-Gopa/Bolabol)
  BOLABOL_DMG_PATTERN  Release asset glob (default: Bolabol*.dmg)
USAGE
}
...
if [[ "$FROM_GITHUB" -eq 1 ]]; then
  ...
  TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bolabol-install.XXXXXX")"
...
# Free a previous Bolabol volume if a prior install left it mounted.
...
if [[ -z "$MOUNT_POINT" || ! -d "$MOUNT_POINT/$APP_NAME.app" ]]; then
  for candidate in "/Volumes/$VOLUME_NAME" /Volumes/Bolabol*; do
...
if [[ ! -x "$DEST/Contents/MacOS/Bolabol" ]]; then
  echo "warning: could not confirm executable Contents/MacOS/Bolabol" >&2
fi
