...
fail=0
API="Sources/NativeBolabolCore/Models/APIProviderSettings.swift"
CLOUD_ENGINE="Sources/NativeBolabol/Services/CloudTextPolishingEngine.swift"
LOCAL_WORKER="Sources/NativeBolabolPolishWorker/main.swift"
REQUEST_POLICY="Sources/NativeBolabolCore/Services/PolishingRequestPolicy.swift"
...
for f in \
  Sources/NativeBolabol/Views/Settings/APIProvidersSettingsView.swift \
  Sources/NativeBolabol/Services/CloudTextPolishingEngine.swift \
  Sources/NativeBolabol/Services/CloudProviderModelCatalog.swift \
  Sources/NativeBolabol/Services/GeminiCloudDictationEngine.swift; do
  if [ ! -f "$f" ]; then
    echo "FAIL: missing $f"
    fail=1
  fi
