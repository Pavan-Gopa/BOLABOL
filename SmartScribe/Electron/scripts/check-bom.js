...
function scan(dir, bad=[]){
  for(const e of fs.readdirSync(dir)){
    ...
    if(s.isDirectory()){ if(!IGNORE.has(e)) scan(f,bad); }
      ...
      if(buf.length>=3 && buf[0]===0xEF && buf[1]===0xBB && buf[2]===0xBF) bad.push(f);
    }
  }
  return bad;
}
const bad = scan(ROOT);
if(bad.length){ console.error("❌ Files with UTF-8 BOM:"); bad.forEach(f=>console.error("  "+f)); process.exit(1); }
else { console.log("✔ No BOM detected"); }