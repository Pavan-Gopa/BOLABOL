...
final class HotkeyOutputDispatcher {
    ...
    func dispatch(
        ...
        guard !trimmed.isEmpty else {
            NativeSmartScribeLog.hotkey.warning("Skipped hotkey output because resolved text is empty.")
            return
        }
        ...
        if dispatchSignature == lastDispatchSignature,
           now.timeIntervalSince(lastDispatchTime) < Self.debounceInterval {
            NativeSmartScribeLog.hotkey.warning(
    ...
    private func pasteIntoActiveApp(
        ...
        if !hasAccessibilityPermission {
            NativeSmartScribeLog.hotkey.warning("Accessibility permission is required to paste hotkey output into the active app.")
        ...
        if let targetApplication {
            ...
            NativeSmartScribeLog.hotkey.warning("No captured target app for hotkey paste; using current frontmost app.")
        }
    ...
    private func attemptAXInsertion(
        ...
        NativeSmartScribeLog.hotkey.info("AX insertion did not modify the field; will fall back to clipboard paste.")
        return false
    }
    ...
    private func resolveInsertionElement(
        ...
        NativeSmartScribeLog.hotkey.warning("Could not resolve any AX element for text insertion.")
        return nil
    }
    ...
    private func insert(_ text: String, into element: AXUIElement, source: String) -> Bool {
        ...
        NativeSmartScribeLog.hotkey.info("Inserted hotkey output through AX value source=\(source, privacy: .public).")
        return true
    }
