...
try {
    ...
    window.addEventListener('error', (e) => {
        ...
        try {
            ipc?.send?.('renderer-error', payload);
        }
        catch { }
    });
    window.addEventListener('unhandledrejection', (e) => {
        ...
        try {
            ipc?.send?.('renderer-unhandledrejection', payload);
        }
        catch { }
    });
}
...
const canLog = (lvl) => LOG_LEVELS[lvl] <= LOG_LEVELS[currentLogLevel];
const logger = {
    setLevel: (lvl) => {
        if (typeof lvl === 'string' && (lvl in LOG_LEVELS))
            currentLogLevel = lvl;
    },
    debug: (message, ...args) => {
        if (canLog('debug'))
    ...
    info: (message, ...args) => {
        if (canLog('info')) {
            ...
            window.ipcRenderer?.send('log-message', { level: 'info', message: String(message), args });
        }
    },
    warn: (message, ...args) => {
        if (canLog('warn')) {
            console.warn(message, ...args);
            window.ipcRenderer?.send('log-message', { level: 'warn', message: String(message), args });
        }
    },
    error: (error, ...args) => {
        console.error(error, ...args);
        const message = error instanceof Error ? `${error.name}: ${error.message}\n${error.stack}` : String(error);
        window.ipcRenderer?.send('log-message', { level: 'error', message, args });
    },
};
// Слушатель уровня логов — БЕЗ деструктурирования второго аргумента
window.ipcRenderer?.on('log-level-changed', (_evt, payload) => {
    const lvl = (payload && typeof payload.level === 'string') ? payload.level : null;
    if (lvl && (lvl in LOG_LEVELS))
        logger.setLevel(lvl);
});
// Экспорт сеттера уровня (для UI/настроек)
window.setAppLogLevel = (level) => window.ipcRenderer?.send('set-log-level', { level });
// --- CONSTANTS ---
...
const UI_LANGUAGE_KEY = 'voiceNotesUiLanguage'; // 'system' | 'en'
...
const I18N = {
    en: {
        ...
        'settings.lang.desc': 'Use your system language when available, or force English.',
...
**Technical Context** (triggers: "prompt", "code", "algorithm", "API", "??????", "???", "????????")
- Structured, precise language
- Technical terminology preserved
- Clear logical flow

**Spiritual Context** (triggers: "Hare Krishna", "devotee", "???? ??????", "?????????", "prabhupada", related terms in any language)
...
### Enhancement
- If the user's speech begins with "rewrite X2", you MUST apply the POLISHED X2 processing level to the rest of the speech.

### Language Handling
- **Auto-detect input language** ? process and output in same language
- **Language modifiers override auto-detection:**
  - "?? ??????????" / "in English" ? English output
  - "?? ???????" / "in Russian" ? Russian output
  - "???" / "in Chinese" ? Chinese output
...
const BASE_LANGUAGES = ['English', 'Russian', 'French', 'Spanish', 'Italian', 'German', 'Arabic', 'Japanese', 'Korean', 'Chinese', 'Hindi'];
...
const OLLAMA_MODEL_BLOCKLIST = new Set([
    'gemma3:1b',
    'gemma2:2b',
    'gemma2:9b',
    'llama3.1:8b',
...
class ApiError extends Error {
    isQuotaError;
    constructor(message, isQuotaError = false) {
        super(message);
...
class CustomSelect {
    ...
    constructor(selectElement) {
        ...
        if (selectElement.nextSibling && selectElement.nextSibling.classList && selectElement.nextSibling.classList.contains('custom-select-container')) {
    ...
    update = () => {
        if (!this.options || !this.trigger || !this.container)
            return;
...
class UniversalApiClient {
    ...
    async generateText(modelIdentifier, prompt) {
        ...
        switch (provider) {
            case 'google':
                if (!this.genAI)
                    throw new ApiError('Google API key is missing. Please add your API key in Settings → Providers.');
                try {
            ...
            case 'openai':
                ...
                return this.fetchApi('https://api.openai.com/v1/chat/completions', openaiSettings.apiKey, {
                    model: modelName,
            ...
            case 'anthropic':
                ...
                return this.fetchApi('https://api.anthropic.com/v1/messages', anthropicSettings.apiKey, {
                    model: modelName,
                    max_tokens: 4096,
                    messages: [{ role: 'user', content: prompt }],
                }, (data) => ({
            ...
            case 'custom':
                ...
                return this.fetchApi(`${customSettings.baseUrl}/chat/completions`, customSettings.apiKey, {
                    model: modelName,
                    messages: [{ role: 'user', content: prompt }],
                    temperature: 0.7,
                }, (data) => ({
            ...
            case 'ollama':
                ...
                const ollamaServerUrl = window.voiceNotesApp?.getOllamaServerUrl?.() || 'http://localhost:11434';
                return this.fetchApi(`${ollamaServerUrl}/api/chat`, '', // Ollama doesn't need API key for local usage
                {
                    model: modelName,
                    messages: [
    ...
    async transcribeAudio(audioBlob, provider, localModelId) {
        this.initializeGemini();
        // 1) Local provider → новый пайплайн PCM→worker
        // ───────── Local provider (no WebAudio decode; WAV parser + safe resample) ─────────
        if (provider === 'local') {
            const w = window;
            ...
            // Check if it's a format supported by Whisper.cpp
            ...
            if (isWav) {
                ...
                // WebM/other format - send to worker for conversion
                logger.debug('[record] Unsupported format for worker, bytes =', ab.byteLength, 'magic =', String.fromCharCode(...new Uint8Array(ab, 0, 4)));
                const language = /-en-/.test(localModelId) ? 'en' : 'auto';
                const res = await w.ipcRenderer.invoke('whisper:transcribeWebM', {
                    webmBytes: new Uint8Array(ab),
                    modelId: localModelId,
                    options: { language, threads: 0, forceCpu: false }
                });
                return { text: res?.text || '', promptTokens: 0, completionTokens: 0 };
        ...
        // This section is now only reachable if provider is 'openai' or 'google'.
        const settings = this.getSettings();
        // Convert blob to base64 for API-based providers - only needed for Gemini
        ...
        if (provider === 'openai') {
            ...
            try {
                ...
                if (!response.ok)
                    throw new ApiError(`Whisper API error: ${response.statusText} - ${await response.text()}`);
        ...
        if (provider === 'google') {
            ...
            // Prefer accurate type when provided; default to WAV which is robust
            const mimeType = audioBlob.type || 'audio/wav';
            // Proper Content[] shape expected by Gemini SDK
            const userContent = [
                {
                    role: 'user',
                    parts: [
                        { text: 'Transcribe this audio to plain text. Return only the transcription without timestamps or extra metadata.' },
                        { inlineData: { mimeType, data: base64Audio } },
                    ],
                },
            ];
            // Try current default model first, then a known-good audio-capable fallback
            ...
            for (const model of tryModels) {
                try {
                    ...
                    logger.warn(`Gemini transcription attempt failed on ${model}: ${msg}`);
                    // On server errors, continue to next model; for hard auth/quota errors, break early
                    if (msg.includes('API key') || msg.includes('authentication') || msg.includes('quota') || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('401') || msg.includes('403') || msg.includes('429')) {
                        break;
                    }
    ...
    async fetchApi(url, apiKey, body, responseMapper, additionalHeaders = {}) {
        const headers = {
...
class VoiceNotesApp {
    ...
    customPromptV1Textarea;
    customPromptV2Textarea;
    // private resetPromptRawButton!: HTMLButtonElement | null;
    resetPromptV1Button;
    resetPromptV2Button;
    promptSubTabButtons;
    promptSubTabContents;
    saveSettingsButton;
    cancelPromptButton;
    closePromptModalButton;
    themeToggleButtonModal;
    themeToggleIconModal;
    settingsTabButtons;
    settingsTabContents;
    apiSettingsInputs;
    targetLanguageSelector;
    localModelsListContainer;
    hotkeyInput;
    hotkeyError;
    ...
    translationResultOverlay;
    closeTranslationResultModalButton;
    // Custom language modal
    customLanguageModal;
    customLanguageModalOverlay;
    closeCustomLanguageModalButton;
    cancelCustomLanguageButton;
    confirmCustomLanguageButton;
    customLanguageGrid;
    customLanguageSelected = new Set();
    customLanguagesSaved = new Set();
    closeTranslationModalButton;
    ...
    modalWorkletNode = null;
    modalStream = null;
    modalAudioBuffer = new Float32Array(0);
    modalRecordingSampleRate = 16000;
    originalTextElement;
    translatedTextElement;
    translationTargetLanguageElement;
    translationProviderSelector = null;
    lastSelectionText = '';
    clipboardText = '';
    OLLAMA_SERVER_URL_KEY = 'ollama_server_url';
    ...
    confirmErrorButton;
    lastTxPromptTokensElement;
    lastTxCompletionTokensElement;
    lastTxTotalTokensElement;
    totalPromptTokensElement;
    totalCompletionTokensElement;
    totalTotalTokensElement;
    resetUsageButton;
    statsModelNameElement;
    openLogsButton;
    logLevelSelector;
    uiZoomSlider;
    uiZoomValue;
    uiLanguageSelector;
    uiLanguagePref = 'system';
    usageStats = {};
    tabButtons;
    activeTabIndicator = null;
    noteContents;
    audioChunks = [];
    isRecording = false;
    stream = null;
    ...
    statusIndicatorDiv = null;
    analyserNode = null;
    waveformDataArray = null;
    waveformDrawingId = null;
    ...
    localModelsState = {};
    localModelProviders = {};
    localModelFallbackReasons = {};
    stagedActiveLocalModelId = null;
    ollamaModelsState = {};
    selectedOllamaModelId = null;
    OLLAMA_MODELS_STATE_KEY = 'ollama_models_state';
    ...
    constructor() {
        try {
            ...
            for (const modelId in this.localModelsState) {
                if (this.localModelsState.hasOwnProperty(modelId)) {
                    ...
                    if (modelState.status === 'downloading') {
                        logger.warn(`Found interrupted download for model ${modelId}. Resetting status to 'failed'.`);
            ...
            this.polishedNoteX2 = document.getElementById('polishedNoteX2');
            this.clearButton = document.getElementById('clearButton');
            this.copyButton = document.getElementById('copyButton');
            this.copyButtonIcon = this.copyButton.querySelector('i');
            this.promptSettingsButton = document.getElementById('promptSettingsButton');
            this.transcriptionProviderSelector = document.getElementById('transcriptionProviderSelector');
            this.modelSelector = document.getElementById('modelSelector');
            this.translateButton = document.getElementById('translateButton');
            this.fileTranscribeButton = document.getElementById('fileTranscribeButton');
            ...
            this.promptModalOverlay = document.getElementById('promptModalOverlay');
            // Query new per-variant prompt fields (may be null if HTML not loaded yet)
            // RAW prompt has been removed from UI
            this.customPromptV1Textarea = document.getElementById('customPromptV1Textarea');
            this.customPromptV2Textarea = document.getElementById('customPromptV2Textarea');
            // RAW reset button removed
            this.resetPromptV1Button = document.getElementById('resetPromptV1Button');
            this.resetPromptV2Button = document.getElementById('resetPromptV2Button');
            this.promptSubTabButtons = document.querySelectorAll('.prompt-subtab-button');
            this.promptSubTabContents = document.querySelectorAll('.prompt-subtab-content');
            ...
            this.closePromptModalButton = document.getElementById('closePromptModalButton');
            this.themeToggleButtonModal = document.getElementById('themeToggleButtonModal');
            this.themeToggleIconModal = this.themeToggleButtonModal.querySelector('i');
            ...
            this.targetLanguageSelector = document.getElementById('targetLanguageSelector');
            this.localModelsListContainer = document.getElementById('localModelsList');
            this.hotkeyInput = document.getElementById('hotkeyInput');
            this.hotkeyError = document.getElementById('hotkeyError');
            this.translationResultModal = document.getElementById('translationResultModal');
            this.translationResultOverlay = document.getElementById('translationResultModalOverlay');
            this.closeTranslationResultModalButton = document.getElementById('closeTranslationResultModalButton');
            this.closeTranslationModalButton = document.getElementById('closeTranslationModalButton');
            this.copyTranslationButton = document.getElementById('copyTranslationButton');
            this.originalTextElement = document.getElementById('originalText');
            this.translatedTextElement = document.getElementById('translatedText');
            this.translationTargetLanguageElement = document.getElementById('translationTargetLanguage');
            this.retranslateButton = document.getElementById('retranslateButton');
            this.modalRecordButton = document.getElementById('modalRecordButton');
            // Custom language modal elements
            this.customLanguageModal = document.getElementById('customLanguageModal');
            this.customLanguageModalOverlay = document.getElementById('customLanguageModalOverlay');
            this.closeCustomLanguageModalButton = document.getElementById('closeCustomLanguageModalButton');
            this.cancelCustomLanguageButton = document.getElementById('cancelCustomLanguageButton');
            this.confirmCustomLanguageButton = document.getElementById('confirmCustomLanguageButton');
            this.customLanguageGrid = document.getElementById('customLanguageGrid');
            this.lastTxPromptTokensElement = document.getElementById('lastTxPromptTokens');
            this.lastTxCompletionTokensElement = document.getElementById('lastTxCompletionTokens');
            this.lastTxTotalTokensElement = document.getElementById('lastTxTotalTokens');
            this.totalPromptTokensElement = document.getElementById('totalPromptTokens');
            this.totalCompletionTokensElement = document.getElementById('totalCompletionTokens');
            this.totalTotalTokensElement = document.getElementById('totalTotalTokens');
            this.resetUsageButton = document.getElementById('resetUsageButton');
            this.statsModelNameElement = document.getElementById('statsModelName');
            this.openLogsButton = document.getElementById('openLogsButton');
            this.logLevelSelector = document.getElementById('logLevelSelector');
            this.uiZoomSlider = document.getElementById('uiZoomSlider');
            this.uiZoomValue = document.getElementById('uiZoomValue');
            this.uiLanguageSelector = document.getElementById('uiLanguageSelector');
            const tabNav = document.querySelector('.tab-navigation');
            this.tabButtons = tabNav.querySelectorAll('.tab-button');
            this.activeTabIndicator = tabNav.querySelector('.active-tab-indicator');
            ...
            this.setupWizardOverlay = document.getElementById('setupWizardOverlay');
            this.wizardApiKeyStatus = document.getElementById('wizardApiKeyStatus');
            this.wizardMicStatus = document.getElementById('wizardMicStatus');
            this.wizardMicButton = document.getElementById('wizardMicButton');
            this.wizardFinishButton = document.getElementById('wizardFinishButton');
            // Wire prompt sub-tab handlers if present
            if (this.promptSubTabButtons) {
                this.promptSubTabButtons.forEach((btn) => btn.addEventListener('click', () => this.setActivePromptSubTab(btn)));
            }
            // Confirmation dialog elements
            this.confirmationModal = document.getElementById('confirmationModal');
            this.confirmationModalOverlay = document.getElementById('confirmationModalOverlay');
            this.closeConfirmationModalButton = document.getElementById('closeConfirmationModalButton');
            this.cancelConfirmationButton = document.getElementById('cancelConfirmationButton');
            this.confirmDeleteButton = document.getElementById('confirmDeleteButton');
            this.confirmationMessage = document.getElementById('confirmationMessage');
            // Error dialog elements
            this.errorModal = document.getElementById('errorModal');
            this.errorModalOverlay = document.getElementById('errorModalOverlay');
            ...
            this.closeErrorModalButton = document.getElementById('closeErrorModalButton');
            this.confirmErrorButton = document.getElementById('confirmErrorButton');
            // Ollama elements
            this.installOllamaButton = document.getElementById('installOllamaButton');
            this.ollamaStatusMessage = document.getElementById('ollamaStatusMessage');
            this.ollamaModelsSection = document.getElementById('ollamaModelsSection');
            this.ollamaModelsList = document.getElementById('ollamaModelsList');
            this.ollamaServerUrlInput = document.getElementById('ollamaServerUrl');
            // Initialize persisted Ollama URL
            try {
                const saved = localStorage.getItem(this.OLLAMA_SERVER_URL_KEY);
                if (saved && this.ollamaServerUrlInput)
                    this.ollamaServerUrlInput.value = this.normalizeOllamaUrl(saved);
            }
            catch { }
            // This check is crucial for debugging silent startup failures
            if (!this.recordButton)
                throw new Error("Element with ID 'recordButton' not found.");
            // --- Custom Select Instantiation ---
            new CustomSelect(this.transcriptionProviderSelector);
            new CustomSelect(this.modelSelector);
            new CustomSelect(document.getElementById('hotkeyTarget'));
            new CustomSelect(document.getElementById('hotkeyMode'));
    ...
    bindEventListeners() {
        ...
        this.themeToggleButtonModal.addEventListener('click', () => this.toggleTheme());
        this.copyButton.addEventListener('click', () => this.copyPolishedNote());
        this.promptSettingsButton.addEventListener('click', () => this.openPromptModal());
        this.saveSettingsButton.addEventListener('click', () => this.saveSettings());
        // Reset prompt buttons
        // RAW reset handler removed
        this.resetPromptV1Button?.addEventListener('click', () => {
            if (this.customPromptV1Textarea)
                this.customPromptV1Textarea.value = DEFAULT_PROMPT_V1;
            try {
                localStorage.removeItem(PROMPT_STORAGE_KEY_V1);
            }
            catch { }
        });
        this.resetPromptV2Button?.addEventListener('click', () => {
            if (this.customPromptV2Textarea)
                this.customPromptV2Textarea.value = DEFAULT_PROMPT_V2;
            try {
                localStorage.removeItem(PROMPT_STORAGE_KEY_V2);
            }
            catch { }
        });
        this.cancelPromptButton.addEventListener('click', () => this.closePromptModal());
        this.closePromptModalButton.addEventListener('click', () => this.closePromptModal());
        this.promptModalOverlay.addEventListener('click', () => this.closePromptModal());
        this.resetUsageButton.addEventListener('click', () => this.resetModelUsage());
        ...
        if (this.logLevelSelector) {
            this.logLevelSelector.addEventListener('change', () => {
                const level = this.logLevelSelector.value;
                localStorage.setItem('appLogLevel', level);
                window.setAppLogLevel?.(level);
            });
        }
        // UI zoom slider
        ...
        if (this.closeCustomLanguageModalButton)
            this.closeCustomLanguageModalButton.addEventListener('click', () => this.hideCustomLanguageModal());
        if (this.cancelCustomLanguageButton)
            this.cancelCustomLanguageButton.addEventListener('click', () => this.hideCustomLanguageModal());
        if (this.customLanguageModalOverlay)
            this.customLanguageModalOverlay.addEventListener('click', () => this.hideCustomLanguageModal());
        if (this.confirmCustomLanguageButton)
            this.confirmCustomLanguageButton.addEventListener('click', () => this.applyCustomLanguage());
        ...
        // Delegated copy for per-chunk buttons across all note areas
        const wrapper = document.querySelector('.note-content-wrapper');
        if (wrapper) {
            wrapper.addEventListener('click', (e) => {
                const target = e.target;
                const btn = target.closest('.copy-bubble-btn');
                if (!btn)
                    return;
                e.stopPropagation();
                const section = btn.closest('.note-section');
                const chunk = section?.querySelector('.note-chunk');
                if (!chunk)
                    return;
                navigator.clipboard.writeText(chunk.innerText || '').catch(() => { });
            });
        }
        ...
        this.ollamaServerUrlInput?.addEventListener('blur', () => {
            ...
            if (this.settingsTabButtons) {
                ...
                if (localLlmTab?.classList.contains('active'))
    ...
    initLogLevelSelector() {
        if (!this.logLevelSelector)
            return;
        const stored = (localStorage.getItem('appLogLevel') || 'warn');
        this.logLevelSelector.value = stored;
        if (logger.setLevel)
            logger.setLevel(stored);
        window.setAppLogLevel?.(stored);
    }
    bindIpcListeners() {
        if (!ipcRenderer)
            return;
        ...
        ipcRenderer.on('download-complete', (event, { id, path }) => {
            const modelState = this.localModelsState[id];
            if (modelState) {
                modelState.status = 'downloaded';
                modelState.path = path;
                this.saveLocalModelsState();
        ...
        ipcRenderer.on('local-model-removed', (event, { id, success, error }) => {
            if (success) {
        ...
        ipcRenderer.on('model-provider-status', (event, { id, provider, error }) => {
            ...
            if (providerChanged || reasonChanged) {
                ...
                this.saveLocalModelProviders();
                this.saveLocalModelFallbackReasons();
                // Re-render if the local models tab is currently visible
                const localModelsTab = document.getElementById('localModelsTabContent');
                if (localModelsTab && localModelsTab.classList.contains('active')) {
                    this.renderLocalModelsList();
                }
            }
        });
    ...
    setupCanvasDimensions() {
        if (!this.liveWaveformCanvas || !this.liveWaveformCtx)
            return;
    ...
    initUiZoom() {
        ...
        document.body.style.zoom = String(saved);
        if (this.uiZoomSlider)
            this.uiZoomSlider.value = String(saved);
        if (this.uiZoomValue)
            this.uiZoomValue.textContent = `${Math.round(saved * 100)}%`;
    }
    applyUiZoom() {
        if (!this.uiZoomSlider)
            return;
        ...
        if (this.uiZoomValue)
            this.uiZoomValue.textContent = `${Math.round(val * 100)}%`;
    }
    ...
    populateUiLanguageSelector() {
        ...
        add(this.t('settings.lang.englishOption'), 'en');
        const saved = localStorage.getItem(UI_LANGUAGE_KEY) || 'system';
        this.uiLanguagePref = (saved === 'en' ? 'en' : 'system');
    ...
    onUiLanguageChanged() {
        if (!this.uiLanguageSelector)
            return;
        const v = this.uiLanguageSelector.value;
        this.uiLanguagePref = (v === 'en' ? 'en' : 'system');
        localStorage.setItem(UI_LANGUAGE_KEY, this.uiLanguagePref);
        this.applyUiLanguage();
    }
    getActiveLocale() {
        if (this.uiLanguagePref === 'en')
            return 'en';
    ...
    applyUiLanguage() {
        // Tabs
        const rawBtn = document.querySelector('.tab-button[data-tab="raw"]');
        const v1Btn = document.querySelector('.tab-button[data-tab="note"]');
        const v2Btn = document.querySelector('.tab-button[data-tab="x2"]');
        if (rawBtn)
            rawBtn.textContent = this.t('tab.raw');
        if (v1Btn)
            v1Btn.textContent = this.t('tab.v1');
        if (v2Btn)
            v2Btn.textContent = this.t('tab.v2');
        // Recording status and button label
        const status = document.getElementById('recordingStatus');
        if (status)
            status.textContent = this.t('record.ready');
        const recordText = document.querySelector('.record-text');
        if (recordText)
            recordText.textContent = this.t('record.button');
        // Record button title
        const recordBtnEl = document.getElementById('recordButton');
        if (recordBtnEl)
            recordBtnEl.setAttribute('title', this.t('tooltip.record'));
        ...
        if (modal) {
            ...
            const tabsMap = {
                ...
                localLlm: 'settings.localLlm', prompt: 'settings.prompt', hotkey: 'settings.hotkey', statistics: 'settings.statistics', help: 'settings.help'
            };
            Object.entries(tabsMap).forEach(([dataTab, k]) => {
                const btn = modal.querySelector(`.modal-tab-button[data-tab="${dataTab}"]`);
                if (btn)
                    btn.textContent = this.t(k);
            });
            const cancel = document.getElementById('cancelPromptButton');
            const save = document.getElementById('saveSettingsButton');
            if (cancel)
                cancel.textContent = this.t('settings.cancel');
            if (save)
                save.textContent = this.t('settings.save');
        }
        ...
        if (transSel)
            transSel.setAttribute('title', this.t('tooltip.transcriptionProvider'));
        const modelSel = document.getElementById('modelSelector');
        if (modelSel)
            modelSel.setAttribute('title', this.t('tooltip.polishingModel'));
        const clearBtn = document.getElementById('clearButton');
        if (clearBtn)
            clearBtn.setAttribute('title', this.t('tooltip.clear'));
        const copyBtn = document.getElementById('copyButton');
        if (copyBtn)
            copyBtn.setAttribute('title', this.t('tooltip.copy'));
        const translateBtn = document.getElementById('translateButton');
        if (translateBtn)
            translateBtn.setAttribute('title', this.t('tooltip.translate'));
        const fileBtn = document.getElementById('fileTranscribeButton');
        if (fileBtn)
            fileBtn.setAttribute('title', this.t('tooltip.transcribeFile'));
        const settingsBtn = document.getElementById('promptSettingsButton');
        if (settingsBtn)
            settingsBtn.setAttribute('title', this.t('tooltip.settings'));
        const themeToggleBtn = document.getElementById('themeToggleButtonModal');
        if (themeToggleBtn)
            themeToggleBtn.setAttribute('title', this.t('tooltip.themeToggle'));
        // Localize dynamic copy bubble buttons
        document.querySelectorAll('button.copy-bubble-btn').forEach(btn => {
            btn.setAttribute('title', this.t('tooltip.copy'));
        });
    }
    ...
    drawLiveWaveform() {
        ...
        const canvas = this.liveWaveformCanvas;
        const logicalWidth = canvas.clientWidth, logicalHeight = canvas.clientHeight;
    ...
    updateLiveTimer() {
        if (!this.isRecording || !this.liveRecordingTimerDisplay)
            return;
    ...
    startLiveDisplay() {
        ...
        if (this.isHotkeyRecording) {
            try {
                ipcRenderer?.send('hotkey-transcription-start');
            }
            catch { }
        }
    }
    stopLiveDisplay() {
        ...
        if (!this.recordingInterface || this.recordingInterface.classList.contains('is-stopping')) {
            console.log('[STOP_LIVE_DISPLAY] Already stopping or interface missing, aborting');
            return;
        }
        console.log('[STOP_LIVE_DISPLAY] Adding is-stopping flag');
        this.recordingInterface.classList.add('is-stopping');
        console.log('[STOP_LIVE_DISPLAY] Checking elements...');
        console.log('[STOP_LIVE_DISPLAY] recordingInterface exists:', !!this.recordingInterface);
        console.log('[STOP_LIVE_DISPLAY] liveRecordingTitle exists:', !!this.liveRecordingTitle);
        console.log('[STOP_LIVE_DISPLAY] liveWaveformCanvas exists:', !!this.liveWaveformCanvas);
        console.log('[STOP_LIVE_DISPLAY] liveRecordingTimerDisplay exists:', !!this.liveRecordingTimerDisplay);
        if (!this.recordingInterface ||
            !this.liveRecordingTitle ||
            !this.liveWaveformCanvas ||
            !this.liveRecordingTimerDisplay) {
            console.log('[STOP_LIVE_DISPLAY] Some elements missing, basic cleanup...');
            if (this.recordingInterface) {
                console.log('[STOP_LIVE_DISPLAY] Removing is-live class');
                this.recordingInterface.classList.remove('is-live');
            }
            console.log('[STOP_LIVE_DISPLAY] Early return');
            return;
        }
        console.log('[STOP_LIVE_DISPLAY] All elements exist, full cleanup...');
        console.log('[STOP_LIVE_DISPLAY] Removing is-live class');
        try {
            this.recordingInterface.classList.remove('is-live');
            console.log('[STOP_LIVE_DISPLAY] is-live class removed successfully');
        }
        catch (error) {
            console.log('[STOP_LIVE_DISPLAY] Error removing is-live class:', error);
            throw error;
        }
        console.log('[STOP_LIVE_DISPLAY] Setting liveRecordingTitle display to none');
        try {
            this.liveRecordingTitle.style.display = 'none';
            console.log('[STOP_LIVE_DISPLAY] liveRecordingTitle display set successfully');
        }
        catch (error) {
            console.log('[STOP_LIVE_DISPLAY] Error setting liveRecordingTitle display:', error);
            throw error;
        }
        console.log('[STOP_LIVE_DISPLAY] Setting liveWaveformCanvas display to none');
        try {
            this.liveWaveformCanvas.style.display = 'none';
            console.log('[STOP_LIVE_DISPLAY] liveWaveformCanvas display set successfully');
        }
        catch (error) {
            console.log('[STOP_LIVE_DISPLAY] Error setting liveWaveformCanvas display:', error);
            throw error;
        }
        console.log('[STOP_LIVE_DISPLAY] Setting liveRecordingTimerDisplay display to none');
        try {
            this.liveRecordingTimerDisplay.style.display = 'none';
            console.log('[STOP_LIVE_DISPLAY] liveRecordingTimerDisplay display set successfully');
        }
        catch (error) {
            console.log('[STOP_LIVE_DISPLAY] Error setting liveRecordingTimerDisplay display:', error);
            throw error;
        }
        console.log('[STOP_LIVE_DISPLAY] Checking statusIndicatorDiv');
        if (this.statusIndicatorDiv) {
            console.log('[STOP_LIVE_DISPLAY] Setting statusIndicatorDiv display to block');
            this.statusIndicatorDiv.style.display = 'block';
            console.log('[STOP_LIVE_DISPLAY] statusIndicatorDiv display set successfully');
        }
        console.log('[STOP_LIVE_DISPLAY] Looking for icon element');
        const iconElement = this.recordButton.querySelector('.record-button-inner i');
        console.log('[STOP_LIVE_DISPLAY] Icon element found:', !!iconElement);
        if (iconElement) {
            console.log('[STOP_LIVE_DISPLAY] Replacing fa-stop with fa-microphone');
            iconElement.classList.replace('fa-stop', 'fa-microphone');
            console.log('[STOP_LIVE_DISPLAY] Icon class replaced successfully');
        }
        console.log('[STOP_LIVE_DISPLAY] Canceling animation frame, ID:', this.waveformDrawingId);
        if (this.waveformDrawingId) {
            cancelAnimationFrame(this.waveformDrawingId);
            console.log('[STOP_LIVE_DISPLAY] Animation frame canceled');
        }
        this.waveformDrawingId = null;
        console.log('[STOP_LIVE_DISPLAY] waveformDrawingId set to null');
        console.log('[STOP_LIVE_DISPLAY] Clearing timer interval, ID:', this.timerIntervalId);
        if (this.timerIntervalId) {
            clearInterval(this.timerIntervalId);
            console.log('[STOP_LIVE_DISPLAY] Timer interval cleared');
        }
        this.timerIntervalId = null;
        console.log('[STOP_LIVE_DISPLAY] timerIntervalId set to null');
        console.log('[STOP_LIVE_DISPLAY] Checking liveWaveformCtx and canvas');
        if (this.liveWaveformCtx && this.liveWaveformCanvas) {
            console.log('[STOP_LIVE_DISPLAY] Clearing canvas rect');
            this.liveWaveformCtx.clearRect(0, 0, this.liveWaveformCanvas.width, this.liveWaveformCanvas.height);
            console.log('[STOP_LIVE_DISPLAY] Canvas rect cleared successfully');
        }
        ...
        console.log('[STOP_LIVE_DISPLAY] Setting analyserNode to null');
        this.analyserNode = null;
        console.log('[STOP_LIVE_DISPLAY] Setting waveformDataArray to null');
        this.waveformDataArray = null;
        console.log('[STOP_LIVE_DISPLAY] Removing is-stopping flag');
        this.recordingInterface.classList.remove('is-stopping');
        console.log('[STOP_LIVE_DISPLAY] Method complete');
    }
        ...
        try {
            ...
            if (this.stream) {
                try {
                    ...
                    logger.warn('[Recording] Error stopping existing stream:', e);
                }
            }
            this.stream = null;
            if (this.audioContext) {
                try {
                    ...
                    logger.warn('[Recording] Error closing existing AudioContext:', e);
                }
                this.audioContext = null;
            }
            ...
            try {
                ...
                if (!gotStream || !this.stream) {
                    const msg = err?.name === 'NotReadableError' || /busy|in use|could not start|resource/i.test(String(err?.message || ''))
            ...
            try {
                ...
                if (track) {
                    track.onended = () => { logger.warn('[Recording] Microphone track ended unexpectedly.'); if (this.isRecording)
                        this.handleMicEnded(); };
                    track.onmute = () => { if (this.isRecording)
                        this.recordingStatus.textContent = 'Microphone muted/unavailable...'; };
                    track.onunmute = () => { if (this.isRecording)
                        this.recordingStatus.textContent = 'Recording...'; };
                }
            }
            catch (e) {
                logger.warn('[Recording] Failed to attach track event listeners:', e);
            }
            // 4) Quick MediaRecorder capability probe (optional)
            this.recordingStatus.textContent = 'Testing recording format...';
            try {
                const probe = new MediaRecorder(this.stream);
                if (probe.state !== 'inactive')
                    probe.stop();
            }
            catch (recorderError) {
                throw new Error('Cannot create MediaRecorder: ' + (recorderError instanceof Error ? recorderError.message : 'Unknown error'));
            }
            ...
            logger.info('[Recording] AudioContext created:', { state: this.audioContext.state, sampleRate: this.audioContext.sampleRate });
            try {
            ...
            if (this.isHotkeyRecording) {
                try {
                    ipcRenderer?.send('hotkey-transcription-end');
                }
                catch { }
    ...
    async handleMicEnded() {
        try {
            await this.stopRecording();
        }
        catch (e) {
            logger.warn('[Recording] Error while stopping after mic ended:', e);
        }
    ...
    async stopRecording() {
        ...
        console.log('[STOP_RECORDING] isRecording:', this.isRecording);
        if (this.isRecording) {
            this.isRecording = false;
            this.recordButton.classList.remove('recording');
            this.recordButton.setAttribute('title', 'Start Recording');
            ...
            if (this.audioWorkletNode) {
                console.log('[STOP_RECORDING] Stopping AudioWorklet...');
                // Create a promise that resolves when we get audio data
                const audioPromise = new Promise((resolve) => {
                    const timeout = setTimeout(() => {
                        console.log('[STOP_RECORDING] Timeout waiting for audio data');
                        resolve(null);
                    ...
                    const messageHandler = (event) => {
                        if (event.data.command === 'audioData') {
                            ...
                            if (audioBuffer.length > 0) {
                                ...
                                console.log('[STOP_RECORDING] Created WAV blob, size:', wavBlob.size);
                                resolve(wavBlob);
                            }
                            else {
                                console.log('[STOP_RECORDING] No audio data received');
                                resolve(null);
                            }
                        }
                    };
                ...
                if (audioBlob && audioBlob.size > 0) {
                    ...
                    try {
                        ...
                        console.error('[STOP_RECORDING] processAudio() failed:', err);
                    ...
                    console.log('[STOP_RECORDING] No audio data captured');
            ...
            console.log('[STOP_RECORDING] Not recording, calling stopLiveDisplay');
            this.stopLiveDisplay();
        }
        console.log('[STOP_RECORDING] Method complete');
    }
    ...
    async sendToWhisper(audioBlob) {
        try {
            ...
            if (selectedValue.startsWith('local:')) {
                ...
                transcriptionProvider = selectedValue;
                localModelId = this.apiSettings.local.activeModelId; // For fallback cases
            }
            const transcriptionData = await this.apiClient.transcribeAudio(audioBlob, transcriptionProvider, localModelId);
            const transcriptionText = transcriptionData.text;
            const isPolishingEnabled = !!this.modelSelector.value;
            if (this.isHotkeyRecording) {
                this.recordingStatus.textContent = 'Processing hotkey...';
                let textToSend = transcriptionText;
                let pResult = null; // standard polish
                let pX2Result = null; // x2 polish
                ...
                if (isPolishingEnabled) {
                    if (this.hotkeySettings.target === 'note') {
                        try {
                            ...
                            if (pResult?.text)
                                textToSend = pResult.text;
                        }
                        catch (e) {
                            logger.error('[HOTKEY] Standard polishing failed, falling back to raw.', e);
                        }
                    }
                        ...
                        try {
                            ...
                            if (pX2Result?.text)
                                textToSend = pX2Result.text;
                        }
                        catch (e) {
                            logger.error('[HOTKEY] X2 polishing failed, falling back to raw.', e);
                        }
                    }
                }
                ...
                // Switch overlay to Processing state while AI work continues
                try {
                    ipcRenderer?.send('hotkey-processing-start');
                }
                catch { }
                this.appendToContentArea(this.rawTranscription, transcriptionText);
                const otherPolishPromises = [];
                if (isPolishingEnabled) {
                    if (!pResult) { // Fetch standard if not already fetched
                        otherPolishPromises.push(this.getPolishedNote('standard', transcriptionText).then(r => { pResult = r; }).catch(() => { pResult = null; }));
                    }
                    if (!pX2Result) { // Fetch x2 if not already fetched
                        otherPolishPromises.push(this.getPolishedNote('x2', transcriptionText).then(r => { pX2Result = r; }).catch(() => { pX2Result = null; }));
                    }
                }
                ...
                if (isPolishingEnabled) {
                    ...
                    if (polishedResult.status === 'fulfilled' && polishedResult.value) {
                        ...
                        this.appendToContentArea(this.polishedNote, '<em>Error during polishing.</em>');
                        handleError(polishedResult.reason);
                    }
                    if (polishedX2Result.status === 'fulfilled' && polishedX2Result.value) {
                        ...
                        this.appendToContentArea(this.polishedNoteX2, '<em>Error during polishing.</em>');
                        handleError(polishedX2Result.reason);
                    }
                }
                else {
    ...
    async getPolishedNote(level, rawTextForPolishing) {
        ...
        try {
            ...
            const prompt = basePrompt.replace('${transcription}', textToPolish);
    ...
    // --- Prompt sub-tabs ---
    setActivePromptSubTab(activeButton) {
    ...
    appendToContentArea(element, newContent) {
        if (!newContent?.trim())
            return;
        const placeholder = element.getAttribute('placeholder') || '';
        const isCurrentlyEmpty = element.classList.contains('placeholder-active') ||
            element.innerHTML.trim() === '' ||
            element.innerHTML.trim() === placeholder;
        const copyBtn = `<button class="copy-bubble-btn" title="${this.t('tooltip.copy')}"><i class="fas fa-copy"></i></button>`;
        const block = `<div class="note-section">${copyBtn}<div class="note-chunk">${newContent}</div></div>`;
        if (isCurrentlyEmpty)
            element.innerHTML = block;
        else
            element.innerHTML += '<hr>' + block;
        element.classList.remove('placeholder-active');
        // Scroll the note content wrapper (the actual scroll container)
        const scroller = document.querySelector('.note-content-wrapper');
        if (scroller) {
    ...
    clearAllContent() {
        [this.rawTranscription, this.polishedNote, this.polishedNoteX2].forEach((el) => {
    ...
    async copyPolishedNote() {
        ...
        if (!elementToCopy || !elementToCopy.innerText || elementToCopy.classList.contains('placeholder-active'))
            return;
        try {
            await navigator.clipboard.writeText(elementToCopy.innerText);
            this.copyButtonIcon.classList.replace('fa-copy', 'fa-check');
            setTimeout(() => this.copyButtonIcon.classList.replace('fa-check', 'fa-copy'), 2000);
        }
        catch (err) {
            logger.error('Failed to copy text: ', err);
        }
    }
    async handleTranscribeAudioFile() {
        try {
            ...
            const wavPath = await window.ipcRenderer.invoke('pick-audio-file'); // main.js: ipcMain.handle('pick-audio-file', ...)
            if (!wavPath)
                return;
            // 3) показываем, куда пишем (Raw)
            const rawTabBtn = document.querySelector('.tab-button[data-tab="raw"]');
            if (rawTabBtn)
                this.setActiveTab(rawTabBtn);
            // 4) пускаем транскрипцию через worker (без auto-download, только установленная модель)
            const result = await window.ipcRenderer.invoke('whisper:transcribeFile', {
                modelId: activeModelId,
                fname_inp: wavPath,
                forceCpu: false,
                threads: 0, // 0 = авто, твой воркер сам включает use_threads, см. transcription.fork.js
            });
            // 5) приводим ответ к тексту и кладём в Raw
            const text = Array.isArray(result?.transcription)
                ? result.transcription.map((seg) => seg[2]).join(' ')
                : (result?.text || '');
            if (text?.trim()) {
    ...
    openPromptModal() {
        ...
        this.apiSettingsInputs.forEach((input) => {
            ...
            if (provider && field) {
                if (el.type === 'checkbox') {
                    el.checked = this.apiSettings[provider][field] || false;
                }
                else {
                    el.value = this.apiSettings[provider][field] || '';
                }
            }
        });
        ...
        // Hotkey settings
        document.getElementById('hotkeyEnabled').checked = this.hotkeySettings.enabled;
        const hotkeyTargetSelect = document.getElementById('hotkeyTarget');
        ...
        if (defaultTab)
            this.setActiveSettingsTab(defaultTab);
    }
    closePromptModal() {
    ...
    saveApiSettings() {
        this.apiSettingsInputs.forEach((input) => {
            ...
            const field = el.dataset.field;
            if (provider && field) {
                this.apiSettings[provider][field] = el.value.trim();
            }
        });
    ...
    updateModelSelector() {
        ...
        if (this.apiSettings.google.apiKey || process.env.API_KEY)
        ...
        if (this.apiSettings.openai.apiKey && this.apiSettings.openai.textModel)
            addOption(`OpenAI ${this.apiSettings.openai.textModel}`, `openai:${this.apiSettings.openai.textModel}`);
        if (this.apiSettings.anthropic.apiKey &&
            this.apiSettings.anthropic.textModel)
            addOption(`Anthropic ${this.apiSettings.anthropic.textModel}`, `anthropic:${this.apiSettings.anthropic.textModel}`);
        if (this.apiSettings.custom.apiKey &&
            this.apiSettings.custom.textModel &&
            this.apiSettings.custom.name)
            addOption(`${this.apiSettings.custom.name} ${this.apiSettings.custom.textModel}`, `custom:${this.apiSettings.custom.textModel}`);
    ...
    updateTranscriptionProviderSelector() {
        ...
        if (settings.google.apiKey || process.env.API_KEY) {
        ...
        const setDefaultTab = () => {
            ...
            const activeButton = document.querySelector(`.tab-button[data-tab="${tabToActivate}"]`);
            if (activeButton) {
    ...
    updateLastTransactionDisplay(p = 0, c = 0) {
    ...
    updateStatsDisplay() {
        ...
        this.totalTotalTokensElement.textContent = (stats.promptTokens + stats.completionTokens).toLocaleString();
    }
    saveUsageStats() {
    ...
    saveHotkeySettings() {
        const enabled = document.getElementById('hotkeyEnabled').checked;
        const target = document.getElementById('hotkeyTarget').value;
    ...
    ensureCustomLanguagesInDropdown() {
        if (!this.targetLanguageSelector)
            return;
        ...
        for (const key of this.customLanguagesSaved) {
            if (!existing.has(key)) {
                ...
                opt.textContent = label;
                if (anchor)
                    this.targetLanguageSelector.insertBefore(opt, anchor);
                else
                    this.targetLanguageSelector.appendChild(opt);
                existing.add(key);
            }
        }
    ...
    saveTranslationSettings() {
        if (this.targetLanguageSelector) {
            ...
            if (sel === 'custom') {
                const input = document.getElementById('customTargetLanguage');
        ...
        if (this.translationProviderSelector) {
            ...
            this.translationSettings.providerId = v || undefined;
        }
        localStorage.setItem(TRANSLATION_SETTINGS_KEY, JSON.stringify(this.translationSettings));
    }
    // Show/hide custom language modal
    showCustomLanguageModal(prefill) {
        if (!this.customLanguageModal || !this.customLanguageModalOverlay)
            return;
        // Build two-column card grid of remaining languages
        this.customLanguageSelected.clear();
        const grid = this.customLanguageGrid || document.getElementById('customLanguageGrid');
        if (grid && this.targetLanguageSelector) {
            ...
            if (want) {
                const match = Array.from(grid.children).find((el) => (el.dataset?.value || '').toLowerCase() === want);
                if (match) {
                    match.classList.add('selected');
                    this.customLanguageSelected.add(want);
                }
            }
        }
    ...
    applyCustomLanguage() {
        ...
        const picks = new Set(this.customLanguageSelected);
        if (typed)
            picks.add(typed.toLowerCase());
        ...
        if (this.targetLanguageSelector) {
            ...
            const existingValues = new Set(Array.from(this.targetLanguageSelector.options).map(o => (o.value || '').toLowerCase()));
            ...
            for (const key of toAdd) {
                ...
                if (!existingValues.has(key)) {
                    const opt = document.createElement('option');
                    opt.value = proper;
                    opt.textContent = proper;
                    if (addBefore)
                        this.targetLanguageSelector.insertBefore(opt, addBefore);
                    else
                        this.targetLanguageSelector.appendChild(opt);
            ...
            let nextValue = null;
            if (last)
                nextValue = ALL_LANGUAGES.find(l => l.toLowerCase() === last) || last;
            else if (Array.from(this.targetLanguageSelector.options).some(o => (o.value || '').toLowerCase() === current))
                nextValue = this.targetLanguageSelector.value;
            else
                nextValue = 'English';
            this.targetLanguageSelector.value = nextValue;
            this.targetLanguageSelector.dispatchEvent(new Event('change'));
            this.translationSettings.targetLanguage = nextValue;
    ...
    updateTranslateButtonState() {
        ...
        const hasLocalWhisper = Object.values(this.localModelsState).some((s) => s?.status === 'downloaded');
        const hasLocalLlm = Object.values(this.ollamaModelsState).some((s) => s?.status === 'downloaded');
    ...
    async startModalRecording() {
        try {
            ...
            if (this.modalAudioContext) {
                try {
                    if (this.modalAudioContext.state !== 'closed')
            ...
            if (this.modalAudioContext.state === 'suspended')
                await this.modalAudioContext.resume();
            ...
            this.modalWorkletNode.port.onmessage = (event) => {
                if (event.data?.command === 'audioData') {
                    this.modalAudioBuffer = event.data.data;
                    this.modalRecordingSampleRate = this.modalAudioContext?.sampleRate || 16000;
                }
            };
    ...
    async stopModalRecording() {
        ...
        try {
            ...
            if (this.modalWorkletNode) {
                const audioPromise = new Promise((resolve) => {
                    ...
                    const handler = (event) => {
                        if (event.data?.command === 'audioData') {
                            clearTimeout(timeout);
                            this.modalWorkletNode.port.removeEventListener('message', handler);
                            const buf = event.data.data;
                            if (buf?.length)
                                resolve(this.createWAVBlob(buf, this.modalRecordingSampleRate));
                            else
                                resolve(null);
                        }
                    };
                    this.modalWorkletNode.port.addEventListener('message', handler);
                });
                this.modalWorkletNode.port.postMessage({ command: 'stop' });
                wavBlob = await audioPromise;
                // Cleanup nodes and stream
                try {
                    this.modalWorkletNode.disconnect();
                }
                catch { }
            ...
            if (!wavBlob || wavBlob.size === 0) {
                if (this.translatedTextElement)
                    this.translatedTextElement.innerHTML = '<em>No audio captured.</em>';
                return;
            }
            ...
            if (provider === 'local' && localModelId) {
                ...
                // Send directly to local worker with translate=true
                const ab = await wavBlob.arrayBuffer();
                const res = await w?.ipcRenderer?.invoke('whisper:transcribeWavBytes', {
                    wavBytes: new Uint8Array(ab),
                    modelId: localModelId,
                    options: { language: targetLang, translate: true, threads: 0, forceCpu: false }
                });
                const tx = { text: res?.text || '', promptTokens: 0, completionTokens: 0 };
                const text = (tx?.text || '').trim();
                if (!text) {
                    if (this.translatedTextElement)
                        this.translatedTextElement.innerHTML = '<em>Transcription returned empty.</em>';
                    return;
                }
                if (this.originalTextElement)
                    this.originalTextElement.textContent = text;
                if (this.translatedTextElement)
                    this.translatedTextElement.innerHTML = '<em>Translating...</em>';
                const target = (() => {
                    const sel = this.targetLanguageSelector?.value;
            ...
            const tx = await this.apiClient.transcribeAudio(wavBlob, provider, localModelId);
            const text = (tx?.text || '').trim();
            if (!text) {
                if (this.translatedTextElement)
                    this.translatedTextElement.innerHTML = '<em>Transcription returned empty.</em>';
            ...
            if (this.translationProviderSelector && !this.translationProviderSelector.value) {
                const localLlm = Object.entries(this.ollamaModelsState).find(([_, s]) => s?.status === 'downloaded');
                if (localLlm) {
            ...
            if (this.translatedTextElement)
                this.translatedTextElement.innerHTML = `<em>${(err?.message || 'Recording failed')}</em>`;
        }
    }
    ...
    async performTranslation(text, targetLanguage) {
        ...
        // Local Whisper is audio-only; show hint in text translation modal
        if (modelIdentifier.startsWith('local-whisper:')) {
            if (this.translatedTextElement) {
                this.translatedTextElement.innerHTML = '<em>Local Whisper models translate audio only. Choose a Local LLM (Ollama) or an API provider for text translation.</em>';
            }
...
${text}`;
    ...
    ensureTranslationProviderSelector() {
        if (!this.translationResultModal)
            return;
        const existing = this.translationResultModal.querySelector('#translationProviderSelector');
        if (existing) {
        ...
        const targetGroup = targetSel ? targetSel.closest('.form-group') : null;
    ...
    populateTranslationProviderOptions() {
        if (!this.translationProviderSelector)
            return;
        const sel = this.translationProviderSelector;
        sel.innerHTML = '';
        const addGroup = (groupLabel, opts) => {
            if (!opts.length)
                return;
        ...
        Object.entries(this.localModelsState).forEach(([id, state]) => {
            if (state?.status === 'downloaded') {
                const isEnglishOnly = /(^|[-_\.])en($|[-_\.])/i.test(id) || /-en-/.test(id);
                if (!isEnglishOnly) {
                    const meta = AVAILABLE_LOCAL_MODELS.find(m => m.id === id);
                    const friendly = meta?.name || id;
                    whisperOptions.push({ value: `local-whisper:${id}`, text: `Local model - ${friendly}` });
                }
            }
        });
        addGroup('API Providers & Cloud Models', apiOptions);
        addGroup('Local LLMs (Ollama)', ollamaOptions);
        addGroup('Local Speech Models (audio only)', whisperOptions);
    ...
    openTranslationModal(originalText, translatedText) {
        if (!this.translationResultModal || !this.translationResultOverlay || !this.originalTextElement || !this.translatedTextElement)
            return;
        ...
        // Default provider: saved provider, else currently selected polishing model
        // Default to the same model selected in the main UI; fall back to previously saved provider
        const defaultProvider = this.modelSelector.value || this.translationSettings.providerId || '';
        if (this.translationProviderSelector) {
            const allowed = Array.from(this.translationProviderSelector.options).map(o => o.value);
            const pick = allowed.includes(defaultProvider) ? defaultProvider : (allowed[0] || '');
            this.translationProviderSelector.value = pick;
            this.translationProviderSelector.dispatchEvent(new Event('change'));
        }
        if (this.targetLanguageSelector) {
    ...
    closeTranslationModal() {
        if (!this.translationResultModal || !this.translationResultOverlay)
            return;
    ...
    showConfirmationDialog(message, onConfirm) {
        if (!this.confirmationModal || !this.confirmationModalOverlay || !this.confirmationMessage)
            return;
    ...
    closeConfirmationModal() {
        if (!this.confirmationModal || !this.confirmationModalOverlay)
            return;
    ...
    showErrorDialog(title, message) {
        if (!this.errorModal || !this.errorModalOverlay || !this.errorModalTitle || !this.errorModalMessage)
            return;
    ...
    closeErrorDialog() {
        if (!this.errorModal || !this.errorModalOverlay)
            return;
    ...
    async copyTranslatedText() {
        if (!this.translatedTextElement || !this.copyTranslationButton)
            return;
        const textToCopy = this.translatedTextElement.innerText;
        if (!textToCopy || this.translatedTextElement.querySelector('em'))
            return;
        try {
    ...
    getOllamaServerUrl() {
        ...
        let customUrl = '';
        try {
            customUrl = localStorage.getItem(this.OLLAMA_SERVER_URL_KEY) || '';
        }
        catch { }
        if (!customUrl)
            customUrl = this.ollamaServerUrlInput?.value?.trim() || '';
        return this.normalizeOllamaUrl(customUrl || 'http://localhost:11434');
    }
    normalizeOllamaUrl(url) {
        try {
            let u = (url || '').trim();
            if (!u)
    ...
    async checkOllamaStatusSilently() {
            ...
            if (state?.status === 'downloading') {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const isDownloaded = downloadedModels.some((downloaded) => downloaded.name === modelId);
                        if (isDownloaded) {
    ...
    updateOllamaButton(state, text, icon) {
        const buttonText = this.installOllamaButton.querySelector('.download-button-text');
        const buttonIcon = this.installOllamaButton.querySelector('i');
        if (buttonText)
        ...
        try {
            ...
            if (response.ok) {
                ...
                AVAILABLE_OLLAMA_MODELS.forEach(model => {
                    const isDownloaded = downloadedModels.some((downloaded) => downloaded.name === model.id);
                ...
                downloadedModels.forEach((serverModel) => {
                    const modelId = serverModel.name;
                    if (OLLAMA_MODEL_BLOCKLIST.has(modelId))
                        return; // never include blocklisted models
    ...
    renderOllamaModelsList() {
        ...
        AVAILABLE_OLLAMA_MODELS.forEach(m => {
            if (!OLLAMA_MODEL_BLOCKLIST.has(m.id))
                allModelIds.add(m.id);
        });
        Object.keys(this.ollamaModelsState).forEach(id => {
            if (!OLLAMA_MODEL_BLOCKLIST.has(id))
                allModelIds.add(id);
        });
        ...
        entries.forEach(({ id: modelId }) => {
            ...
            const curatedModel = AVAILABLE_OLLAMA_MODELS.find(m => m.id === modelId);
            const state = this.ollamaModelsState[modelId] || { status: 'not_downloaded' };
            ...
            switch (state.status) {
                ...
                case 'downloading':
                    ...
                    actionHtml = `<button class="download-button downloading" data-model-id="${modelInfo.id}">
                         <div class="progress-bar-fill" style="width: ${progress}%"></div>
                         <div class="download-button-content-wrapper">
                           <i class="fas fa-times"></i>
                           <span class="download-button-text">Downloading...</span>
                         </div>
                       </button>`;
                    break;
                case 'downloaded':
      ...
      `;
            ...
            const actionButtons = item.querySelectorAll('.download-button');
            actionButtons.forEach(button => {
                button.addEventListener('click', (e) => this.handleOllamaModelAction(e, modelInfo, state));
            });
        });
    }
    async handleOllamaModelAction(event, model, state) {
        event.stopPropagation();
        const button = event.currentTarget;
        const buttonText = button.querySelector('.download-button-text');
        const currentText = buttonText?.textContent || '';
        if (currentText === 'Download' || currentText === 'Retry') {
            ...
            this.showConfirmationDialog(`Are you sure you want to delete the model "${model.name}"? This will remove it from your Ollama server.`, () => this.deleteOllamaModel(model));
        }
    }
    async downloadOllamaModel(model) {
        const serverUrl = this.getOllamaServerUrl();
        try {
            ...
            this.saveOllamaModelsState();
            this.renderOllamaModelsList();
            // Use fetch to call ollama pull API
            const response = await fetch(`${serverUrl}/api/pull`, {
                ...
                body: JSON.stringify({
                    name: model.id,
                    stream: false // We'll implement progress tracking later
                })
            });
            if (response.ok) {
                ...
                this.saveOllamaModelsState();
                this.renderOllamaModelsList();
                // Update model selector to include new Ollama model
                this.updateModelSelector();
    ...
    setupTabs() {
        this.tabButtons.forEach((button) => button.addEventListener('click', (e) => this.setActiveTab(e.currentTarget)));
    ...
    setActiveTab(activeButton, skipAnimation = false) {
    ...
    equalizeSettingsTabWidths() {
        const buttons = Array.from(this.settingsTabButtons || []);
        if (!buttons.length)
            return;
    ...
    setActiveSettingsTab(activeButton) {
        const tabName = activeButton.getAttribute('data-tab');
        this.settingsTabButtons.forEach((button) => button.classList.toggle('active', button === activeButton));
        this.settingsTabContents.forEach((content) => content.classList.toggle('active', content.id === `${tabName}TabContent`));
        const isPromptTab = tabName === 'prompt';
        const isProvidersTab = tabName === 'providers';
    const isStatsTab = tabName === 'statistics';
        const isHotkeyTab = tabName === 'hotkey';
        const isLocalModelsTab = tabName === 'localModels';
        const isLocalLlmTab = tabName === 'localLlm';
        const footerRight = this.promptModal.querySelector('.footer-right');
        if (footerRight)
    ...
    async handleDownloadClick(e, model) {
        e.preventDefault();
        e.stopPropagation();
        const w = window;
    ...
    handleRemoveClick(e, model) {
        e.stopPropagation();
        this.showConfirmationDialog(`Are you sure you want to remove the model "${model.name}"? This will delete the files from your computer.`, async () => {
            try {
                const hasInvoke = typeof window.whisper?.removeModel === 'function';
                if (hasInvoke) {
                    const res = await window.whisper.removeModel(model.id);
                    if (res?.ok) {
                        ...
                        if (this.localModelProviders[model.id])
                            delete this.localModelProviders[model.id];
                        if (this.localModelFallbackReasons[model.id])
                            delete this.localModelFallbackReasons[model.id];
                        if (this.apiSettings.local.activeModelId === model.id)
                            this.apiSettings.local.activeModelId = null;
                        if (this.stagedActiveLocalModelId === model.id)
                            this.stagedActiveLocalModelId = null;
                        this.saveLocalModelsState();
                    ...
                    this.showErrorDialog('Delete Failed', res?.error || 'Could not remove the model files.');
                }
                else if (ipcRenderer) {
                ...
                this.showErrorDialog('Delete Failed', err?.message || String(err));
            }
        });
    }
    getComputeTagHtml(modelId) {
        ...
        if (provider === 'vulkan') {
            return `<div class="meta-tag compute-gpu"><i class="fas fa-microchip"></i> GPU: Vulkan</div>`;
        }
        if (provider === 'metal') {
            return `<div class="meta-tag compute-gpu"><i class="fas fa-microchip"></i> GPU: Metal</div>`;
        }
        if (provider === 'failed') {
            const sanitizedReason = fallbackReason?.replace(/"/g, '&quot;').replace(/>/g, '&gt;').replace(/</g, '&lt;') || 'Unknown error';
            const tooltipText = `Model loading failed. Reason: ${sanitizedReason}`;
            return `<div class="meta-tag compute-failed"><i class="fas fa-exclamation-triangle"></i> Failed <i class="fas fa-info-circle" title="${tooltipText}"></i></div>`;
        }
        // Default state before model is loaded
        return `<div class="meta-tag"><i class="fas fa-microchip"></i> CPU</div>`;
    }
    renderLocalModelsList() {
        if (!this.localModelsListContainer)
            return;
        this.localModelsListContainer.innerHTML = ''; // Clear previous list
        AVAILABLE_LOCAL_MODELS.forEach(model => {
            const state = this.localModelsState[model.id] || { status: 'not_downloaded', progress: 0, path: null };
            const isActive = this.stagedActiveLocalModelId === model.id;
            const isWebPreview = !window.__ELECTRON__ ||
                !window.ipcRenderer;
            const item = document.createElement('div');
            ...
            switch (state.status) {
                case 'not_downloaded':
                    actionHtml = isWebPreview
                        ? `<button class="download-button" disabled title="Unavailable in web preview">
         <div class="download-button-content-wrapper">
           <i class="fas fa-download"></i>
           <span class="download-button-text">Download</span>
         </div>
         ...
         <div class="download-button-content-wrapper">
           ...
           <span class="download-button-text">Download</span>
                    ...
                    break;
                case 'downloading':
                    actionHtml = `<button class="download-button downloading" disabled>
                                <div class="download-button-content-wrapper">
                ...
                case 'failed':
                    actionHtml = isWebPreview
                        ? `<button class="download-button retry" disabled title="Unavailable in web preview">
         <div class="download-button-content-wrapper">
           <i class="fas fa-sync-alt"></i>
           <span class="download-button-text">Retry</span>
         </div>
           ...
           <span class="download-button-text">Retry</span>
            ...
            <div class="model-details">
                <div class="model-title-row">
                    ...
                    ${isActive ? `<span class="model-badge active-indicator">Active</span>` : ''}
                </div>
                <p class="model-description">${model.description}</p>
                <div class="model-meta">
                    <div class="meta-tag"><i class="fas fa-crosshairs"></i> Accuracy ${ratingDots(model.accuracy)}</div>
                    <div class="meta-tag"><i class="fas fa-bolt"></i> Speed ${ratingDots(model.speed)}</div>
                    <div class="meta-tag"><i class="fas fa-hdd"></i> ${model.size}</div>
                    <div class="meta-tag"><i class="fas fa-globe"></i> ${model.language}</div>
                    ${this.getComputeTagHtml(model.id)}
                </div>
            </div>
        ...
        `;
            ...
            item.addEventListener('click', (e) => {
                // Don't handle item clicks if clicking on buttons
                if (e.target.closest('.download-button')) {
                    return;
                }
                this.handleLocalModelClick(model, state);
            });
    ...
    bindWizardEventListeners() {
        if (!this.wizardMicButton || !this.wizardFinishButton)
            return;
    ...
    checkWizardApiKeyStatus() {
        if (!this.wizardApiKeyStatus)
            return;
        const isApiConfigured = (this.apiSettings.google.apiKey || process.env.API_KEY) ||
    ...
    async checkWizardMicStatus(promptUser) {
        if (!this.wizardMicStatus || !this.wizardMicButton)
            return;
