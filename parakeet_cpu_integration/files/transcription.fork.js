...
async function ensureModelFiles(modelId) {
  ...
  const run = (async () => {
    for (let i=0;i<missing.length;i++) {
      ...
      process.send?.({ type: 'download_progress', fileName: name, currentFile: i+1, totalFiles: missing.length, progress: 0 });
      const maxAttempts = 3; let attempt = 0;
      while (attempt < maxAttempts) {
        try {
          await downloadFile(url, path.join(dir, name), (done,total) => {
            const progress = total>0 ? Math.round(done*100/total) : -1;
            process.send?.({ type: 'download_progress', fileName: name, currentFile: i+1, totalFiles: missing.length, progress });
          });
          break;
        } catch (e) {
          attempt++;
